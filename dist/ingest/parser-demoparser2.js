...
async function parseDemoWithDemoparser2(demoPath, verbose) {
    ...
    if (wantedTicks.length > 0 && wantedPlayers.length > 0) {
        const tickRows = toArray(parserModule.parseTicks(demoPath, ["tick", "yaw", "pitch", "steamid"], wantedTicks, wantedPlayers, false, false));
    ...
    if (verbose) {
        warnings.push(`[verbose] Parsed events: players=${playersBySlot.size}, kills=${kills.length}, shots=${shots.length}, frames=${frames.length}, roundEnds=${roundEndEvents.length}`);
    }
    return {
        parser: "demoparser2",
...
function deriveRoundCount(roundEnds, kills, shots) {
...
function deriveTotalTicks(roundEnds, kills, shots, frames) {
    let maxTick = -1;
