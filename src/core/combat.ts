import type { PlayerCombatSummary } from "../domain/types";
import type { ParsedDamage, ParsedKill } from "../ingest/types";

export function computePlayerCombatSummary(
  playerSlot: number,
  kills: ParsedKill[],
  ...
  const headshotKills = kills.filter(
    (kill) => kill.attackerSlot === playerSlot && kill.headshot,
  ).length;
